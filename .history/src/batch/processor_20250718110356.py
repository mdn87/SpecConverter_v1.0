...
class BatchProcessor:
    ...
    def _process_single_file(self, job: BatchJob, file_path: str, extract_only: bool, validate_only: bool):
        ...
        extractor.save_to_json(extracted_data, str(json_output_path))
        
        # Save modular files if requested
        if job.options.get('save_modular_files', True):
            extractor.save_modular_json_files(extracted_data, base_name, job.output_dir)
        ...
        if not extract_only and not validate_only:
            ...
            # Use the CLI generate command approach
            ...
            doc = Document(job.template_path)
            clone_header_footer_styles(doc, doc)
            clear_document(doc)
            apply_document_settings_from_json(doc, extracted_data)
            apply_margins_from_json(doc, extracted_data)
            apply_style_definitions_from_json(doc, extracted_data)
            generate_content_from_v3_json(doc, extracted_data)
