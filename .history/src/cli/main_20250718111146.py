...
def pdf_extract_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        json_data = {
            ...
            "header_footer": {
                "header_text": result.header_footer.header_text,
                "footer_text": result.header_footer.footer_text,
                "page_numbers": result.header_footer.page_numbers
            },
            "template_analysis": {
                "numbering_scheme": result.template_analysis.numbering_scheme,
                "section_patterns": result.template_analysis.section_patterns,
                "template_path": result.template_analysis.template_path
            },
            "validation_results": {
                ...
                "corrections_applied": result.validation_results.corrections_applied
            }
        }
        ...
        if args.save_modular:
            ...
            for i, block in enumerate(result.content_blocks):
                block_file = modular_dir / f"block_{i+1:03d}_{block.section_number.replace(' ', '_')}.json"
