...
class HybridAnalyzer:
    ...
    def _clean_text_for_comparison(self, text: str) -> str:
        """Clean text for comparison while preserving structural information."""
        # Normalize quotes and dashes
        text = text.replace('"', '"').replace('"', '"').replace('–', '-').replace('—', '-')
        
        # Preserve line breaks but normalize other whitespace within lines
        lines = text.split('\n')
        cleaned_lines = []
        for line in lines:
    ...
    def _extract_numbering_from_pdf_context(self, pdf_match: Tuple[int, int]) -> Optional[str]:
        """Extract numbering from PDF context using line-based analysis."""
        start_pos, end_pos = pdf_match
        
        # Get the line containing our matched text
        ...
        # Look at previous lines for numbering context
        numbering_candidates = []
        
        # Check current line and previous 3 lines for numbering
        for line_offset in range(-3, 1):  # -3, -2, -1, 0
            line_idx = current_line_num + line_offset
            if 0 <= line_idx < len(lines):
                line = lines[line_idx]
                candidates = self._extract_numbering_from_line(line, line_offset)
                numbering_candidates.extend(candidates)
        
        # Return the best candidate based on priority and position
        if numbering_candidates:
            # Sort by priority (highest first), then by line proximity (closest first)
            numbering_candidates.sort(key=lambda x: (x['priority'], -x['line_offset']), reverse=True)
            return numbering_candidates[0]['numbering']
        
        return None
    
    def _extract_numbering_from_line(self, line: str, line_offset: int) -> List[Dict[str, Any]]:
        """Extract numbering candidates from a single line."""
        candidates = []
        ...
        for pattern, priority in patterns:
            ...
            for match in matches:
                ...
                if self._is_complete_numbering(numbering, line, match.start()):
                    candidates.append({
                        'numbering': numbering,
                        'priority': priority,
    ...
    def _calculate_numbering_confidence(self, numbering: str, block: ContentBlock) -> float:
        ...
        # Bonus for line-based context (if numbering appears at start of line)
        if block.text and block.text.strip().startswith(numbering):
            confidence += 0.1
        ...
        # Penalty for numbering that appears in the middle of text (likely not a header)
        if block.text and numbering in block.text and not block.text.strip().startswith(numbering):
            confidence -= 0.2
